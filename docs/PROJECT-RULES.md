...
> This is the single source of truth — all AI tools (Gemini, Cursor, Copilot) read this file.
...
### Package Rules
- 1 feature = 1 package containing controller, service (class hoặc interface + impl), repository, entity, DTOs
...
### Quy tắc Interface & Implementation
- **Nếu service có khả năng có nhiều implementation:** Dùng Interface + Impl. (Ví dụ: `InventoryService` / `InventoryServiceImpl`)
- **Nếu chỉ có một implementation:** Có thể chỉ dùng class. (Ví dụ: `EmailTemplateFormatterService` không cần interface nếu chỉ có 1 cách implement).
...
### Rules
- Nếu dùng interface: Service interface defines the contract — NO implementation details
- Nếu dùng interface: Interface contains ONLY method signatures, no default methods (unless shared logic)
- `@Service` annotation luôn đặt ở Implementation class, KHÔNG đặt ở interface
...
- Repository **không gọi Service** (để giữ dependency sạch)
...
- Không log business ở Controller. Chỉ log nếu có auditing hoặc tracing. Trong production, controller vẫn có thể cần log request ID hoặc correlation ID.
...
Ưu tiên **Single Responsibility**. Nếu vượt giới hạn thì xem xét refactor.
(Lưu ý: Có những service xử lý Checkout hoặc Specification Builder hoàn toàn có thể dài hơn).

| Metric | Limit | Action if exceeded |
|---|---|---|
| File | ~ 300 lines | Xem xét refactor hoặc split class |
| Method | ~ 50 lines | Xem xét extract methods |
...
- [ ] Tuân thủ Single Responsibility, refactor file/method quá dài nếu cần
...
- **Phân biệt rõ 2 loại validation:**
  - **Syntax Validation (ở Controller):** Sử dụng các annotation của Jakarta Bean Validation (VD: `@NotNull`, `@NotBlank`, `@Email`, `@Size`).
  - **Business Validation (ở Service):** Kiểm tra các quy tắc nghiệp vụ (VD: `Stock > 0`, email đã tồn tại trong DB chưa).

---

## 19. Transaction

- `@Transactional` **chỉ đặt ở Service**.
- **Controller**: Không sử dụng Transaction.
- **Repository**: Không sử dụng Transaction.
- **Query / Read-only methods**: Đặt `@Transactional(readOnly=true)`.

---

## 20. Event-Driven Architecture (EDA)

- Khi architecture chuyển sang Event-Driven, **hạn chế Service gọi trực tiếp nhiều module khác**.
- **Ví dụ luồng đúng:**
  - Service `Checkout` xử lý xong -> publish `OrderCreatedEvent`
  - Các Listener độc lập xử lý event: `InventoryListener`, `NotificationListener`, `VoucherListener`.

---

## 21. Mapper (MapStruct)

- Dùng **MapStruct** cho việc mapping.
- Luồng chuẩn: **Entity -> Mapper -> DTO**
- KHÔNG gọi `new Response(...)` rải rác khắp nơi trong Service/Controller nếu đã thống nhất dùng MapStruct.
