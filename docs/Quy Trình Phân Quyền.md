...
@PreAuthorize("hasRole('MANAGER') or #id == authentication.principal.claims['userId']")
@GetMapping("/users/{id}")
...
**Khi nào dùng:** Hệ thống cần admin quản lý quyền linh hoạt, nhiều role khác nhau, quyền thay đổi thường xuyên. Ví dụ: ERP, CRM, E-Commerce platform.
...
3. **Contextual Check (Kiểm tra theo ngữ cảnh):**
  "User role MANAGER có quyền gọi PUT /api/v1/users không?" → **Có** (RBAC cho phép).
  "Nhưng MANAGER chỉ được sửa thông tin user thuộc phòng ban của mình?" → **Phải check bằng logic code**.
...
public UserResponse updateUser(Long id, UpdateUserRequest request) {
    ...
    // MANAGER chỉ sửa user cùng department
    if (currentUser.hasRole("MANAGER") 
        && !currentUser.getDepartmentId().equals(target.getDepartmentId())) {
...
| 3. DB-driven Permission | Vừa → Lớn | Ngay lập tức (qua UI) | ⭐⭐⭐ | ERP, CRM, E-Commerce |
